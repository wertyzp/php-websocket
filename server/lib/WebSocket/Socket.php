...
class Socket {
    ...
    private function createSocket($host, $port) {
        $protocol = ($this->ssl === true) ? 'ssl://' : 'tcp://';
        $url = $protocol . $host . ':' . $port;
        $methodBitmask = STREAM_CRYPTO_METHOD_SSLv23_SERVER;
        $options = array( 
            "ssl" => array(
    ...
    private function applySSLContext() {
        $pem_file = './server.pem';
        $pem_key = './key.pem';
        $pem_passphrase = $this->pkPassphrase;
        ...
        stream_context_set_option($this->context, 'ssl', 'local_pk', $pem_key);
